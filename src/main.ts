...
function sel() {
    ...
    var items = root.selectAll('p.item').data(data);
    ...
    items.enter().append('p')
         ...
         .on('click', function (d) {
            ...
            //d3.select(this).transition().duration(2000).style('background-color', '#fff3f3');
            selected = d;
            sel();
         });
    ...
    items.transition()
        .duration(500)
        .style('background-color', (item:any) =>  (item == selected) ? '#999' : '#fff' )
        .style('color', (item:any) =>  (item == selected) ? '#fff' : '#000' )
}
